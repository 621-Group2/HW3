...
show_metrics <- function(model, train, test) {
  ...
  confusionMatrix(test$target, y_pred_model, positive = "1", mode="everything" ) 
...
#show_metrics(res.bestglm$BestModel, dev_train_T, dev_test_T) 
...
As a baseline, the first model build will be a logistic regression model using all provided, untransformed predictor variables provided.
...
anova(model3)
```
__AIC (Akaike Information Criterion) for Model 3__ = `r AIC(model3)`  
__BIC (Bayesian Information Criterion) for Model 3__ = `r BIC(model3)`  

#### Model3 - Model2 Comparison
Since Model2 was a better choice than Model1, we compared Model3 to it using a Chi-square test. The tow models do not appear to have signifcant differences, however Model3 has a lower AIC value.
...
#### Using Alkaike Information Criterion (BIC)
...
The resulting BIC model uses `nox`, `age`, `rad`, and `tax` as the final set of predictors.  All are significantly significant.  
